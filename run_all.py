...
COL_MAPS = {
    ...
    "probate":        {"name":["Style","Name"],"case":["Case #","Case Number"],"date":["Date/Time Enter","Date Filed"],"case_type":["Case Type"],"address":["Address"],"amount":["Amount"]},
    ...
    "tax_deeds":      {"name":["Style","Party Name","name"],"case":["Case #","Case Number","Instrument"],"date":["Date/Time Enter","Date Filed","date_filed"],"case_type":["Case Type","Doc Type"],"address":["Address"],"amount":["Opening Bid","Amount"]},
    ...
    for county_key, county in config.COUNTIES.items():
        ...
        for sig_key, tab_name in config.TABS.items():
            ...
            for row in rows[1:]:
                ...
                heat       = "hot" if score >= config.HOT_LEAD_THRESHOLD else "warm" if score >= 40 else "cold"
        ...
        loaded = len([l for l in all_leads if l["county"]==county_name])
        print(county_name + ": " + str(loaded) + " total records loaded")
...
def write_dashboard_json(county_results, total_new, all_leads):
    ...
    for county_key, county in config.COUNTIES.items():
        ...
        by_county[name] = {
            ...
            "breakdown": {sig: len([l for l in leads if l["signal"]==sig]) for sig in config.SIGNAL_LABELS}
        }

    with open("data/leads.json","w") as f:
        json.dump({"updated":datetime.now().strftime("%Y-%m-%d %H:%M UTC"),
                   "total_new":total_new,"total_records":len(all_leads),
                   "hot_count":hot,"by_county":by_county,"leads":all_leads},f,indent=2)

    print("\nDashboard: "+str(len(all_leads))+" records | "+str(hot)+" hot | "+str(total_new)+" new today")

def send_email(county_results, total_new, elapsed, total_records, hot_count):
    if not config.EMAIL_PASSWORD: return
    lines = ["FL Property Intel — "+datetime.now().strftime("%Y-%m-%d"),"",
    ...
    try:
        msg = EmailMessage()
        msg.set_content("\n".join(lines))
        msg["Subject"] = "FL Intel — "+str(total_new)+" new | "+str(hot_count)+" hot | "+str(total_records)+" on file"
        msg["From"] = config.ALERT_EMAIL
        msg["To"]   = config.ALERT_EMAIL
        with smtplib.SMTP("smtp.gmail.com",587) as s:
            s.starttls(); s.login(config.ALERT_EMAIL,config.EMAIL_PASSWORD); s.send_message(msg)
        print("Email sent")
    except Exception as e:
...
def main():
    start = datetime.now()
    print("="*50+"\nFL PROPERTY INTEL — "+start.strftime("%Y-%m-%d %H:%M")+"\n"+"="*50)
    ...
    print("="*50+"\nDONE — "+str(total_new)+" new | "+str(len(all_leads))+" total | "+str(hot_count)+" hot | "+str(elapsed)+"s\n"+"="*50)
